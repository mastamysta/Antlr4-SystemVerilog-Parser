...
{
public:
    ModuleSimulator(const SignalMap& signals): m_signals_a(signals), m_signals_b(signals)
    { 
    }
    ...
    void simulate(std::size_t cycles)
    {
        ...
        set_signal_eval("rstn", [](SignalMap){ return 0; });

        advance_time();

        set_signal_eval("rstn", [](SignalMap){ return 1; });
...
private:
    uint64_t m_time = 0;
    bool m_clk_val = 0;
    SignalMap m_signals_a; // Initial state of system.
    ...
    void advance_time()
    {
        ...
        current_state["clk"].m_eval = [curr_clk](SignalMap) { return curr_clk; };
        ...
        for (const auto& dependent: current_state["clk"].m_dependent_signals)
        {
            ...
            next_state[dependent].m_eval = [next_value](SignalMap){ return next_value; };
